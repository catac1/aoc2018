...
attack :: Int -> Int -> Int -> Vector Cell -> Vector Cell
attack w h src cells0 =
  ...
  where
    ...
    reduceEnemyHp (Goblin hp) = Goblin (hp - 3)
...
simulateMove :: Int -> Int -> (Round, Vector Cell) -> (Round, Vector Cell)
simulateMove w h (_, cells0) =
...
part1 = interactS "input15.txt" $ \lines -> do
  let cells      = parse lines
      w          = length $ lines !! 0
      h          = length lines
      sims       = iterate (simulateMove w h) (Initial, cells)
...
testRounds = do
  let inp =
      ...
      w          = length $ inp !! 0
      h          = length inp
      moveCellss = iterate (simulateMove w h) (Incomplete, cells)
      indexes    = [0..47] :: [Int]
